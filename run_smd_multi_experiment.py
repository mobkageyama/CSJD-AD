#!/usr/bin/env python3
"""
Multi-experiment runner for SMD dataset
Runs experiments on all 28 machine files and averages the results
"""
...
def setup_logging(seed):
    ...
    log_file = f"results/SMD/seed_{seed}/logs/smd_multi_experiment_{timestamp}.log"
    
    # Create results directory if it doesn't exist
    os.makedirs(f"results/SMD/seed_{seed}/logs", exist_ok=True)
...
def run_single_experiment(machine_file, seed, logger):
    """Run experiment on a single machine file"""
    logger.info(f"Starting experiment for {machine_file} with seed {seed}")
    
    # Create a temporary config file for this experiment
    with open('configs/SMD.yaml', 'r') as f:
        config = yaml.safe_load(f)
    
    # Update config for this specific machine file and seed
    config['data']['smd']['filename'] = machine_file
    config['experiment']['name'] = f"smd_{machine_file}_seed_{seed}_experiment"
    config['random_seed'] = seed
    
    # Update result paths to include seed
    config['logging']['log_dir'] = f"./results/SMD/seed_{seed}/logs"
    config['logging']['model_save_dir'] = f"./results/SMD/seed_{seed}/models"
    config['logging']['prediction_save_dir'] = f"./results/SMD/seed_{seed}/predictions"
    
    # Save temporary config
    temp_config_path = f'configs/SMD_{machine_file}_seed_{seed}.yaml'
    ...
    try:
        ...
        if result.returncode != 0:
            logger.error(f"Experiment failed for {machine_file}")
            logger.error(f"Error output: {result.stderr}")
            return None
        
        logger.info(f"Experiment completed successfully for {machine_file}")
        
        # Immediately copy results to prevent overwriting by next experiment
        # The actual predictions are saved in a nested SMD directory due to how run.py handles paths
        predictions_dir = f"results/SMD/seed_{seed}/SMD/predictions"
        temp_predictions_dir = f"results/SMD/seed_{seed}/temp_predictions_{machine_file}"
        ...
        if os.path.exists(predictions_dir):
            ...
            logger.info(f"Copied results for {machine_file} to {temp_predictions_dir}")
        else:
            logger.error(f"Predictions directory not found: {predictions_dir}")
            logger.info(f"Looking for alternative prediction locations...")
            # Try alternative path without nested SMD
            alt_predictions_dir = f"results/SMD/seed_{seed}/predictions"
        ...
        return parse_experiment_results_from_dir(machine_file, temp_predictions_dir, logger)
        
    except subprocess.TimeoutExpired:
        logger.error(f"Experiment timed out for {machine_file}")
        return None
    except Exception as e:
        logger.error(f"Error running experiment for {machine_file}: {e}")
        return None
    finally:
...
def parse_experiment_results_from_dir(machine_file, predictions_dir, logger):
    ...
    try:
        ...
        if not pred_files:
            logger.error(f"No prediction results found for {machine_file} in {predictions_dir}")
            return None
        ...
        logger.info(f"Individual results for {machine_file}: AUC={individual_metrics['auc']:.4f}, "
        ...
        logger.error(f"Error parsing results for {machine_file}: {e}")
        return None
...
def aggregate_results(results_list, logger):
    ...
    for result in valid_results:
        individual_stats.append({
            'machine_file': result['machine_file'],
...
def save_aggregated_results(aggregated_result, seed, logger):
    ...
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    results_dir = f"results/SMD/seed_{seed}"
    
    # Create results directory if it doesn't exist
    os.makedirs(results_dir, exist_ok=True)
    
    # Save detailed JSON results
    json_file = f"{results_dir}/smd_multi_experiment_results_seed_{seed}_{timestamp}.json"
    ...
    cm_file = f"{results_dir}/smd_confusion_matrix_seed_{seed}_{timestamp}.csv"
    ...
    final_summary_file = f"{results_dir}/smd_final_summary_seed_{seed}_{timestamp}.csv"
    ...
    individual_csv = f"{results_dir}/smd_individual_results_seed_{seed}_{timestamp}.csv"
...
def cleanup_temp_directories(machine_files, seed, logger):
    """Clean up temporary prediction directories"""
    logger.info("Cleaning up temporary directories...")
    
    import shutil
    for machine_file in machine_files:
        temp_dir = f"results/SMD/seed_{seed}/temp_predictions_{machine_file}"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run multi-experiment for SMD dataset.")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for experiments.")
    args = parser.parse_args()

    logger = setup_logging(args.seed)
    logger.info(f"Starting SMD multi-experiment runner with seed {args.seed}")
    
    # List of all machine files (machine-1-1 to machine-1-8, machine-2-1 to machine-2-9, machine-3-1 to machine-3-11)
    machine_files = []
    
    # Machine group 1: machine-1-1 to machine-1-8
    machine_files.extend([f"machine-1-{i}" for i in range(1, 9)])
    
    # Machine group 2: machine-2-1 to machine-2-9
    machine_files.extend([f"machine-2-{i}" for i in range(1, 10)])
    
    # Machine group 3: machine-3-1 to machine-3-11
    machine_files.extend([f"machine-3-{i}" for i in range(1, 12)])
    
    logger.info(f"Will run experiments on {len(machine_files)} machine files:")
    logger.info(f"  Machine Group 1: {[f'machine-1-{i}' for i in range(1, 9)]}")
    logger.info(f"  Machine Group 2: {[f'machine-2-{i}' for i in range(1, 10)]}")
    logger.info(f"  Machine Group 3: {[f'machine-3-{i}' for i in range(1, 12)]}")
    logger.info(f"Using random seed: {args.seed}")
    logger.info(f"Results will be saved to: results/SMD/seed_{args.seed}/")
    ...
    for machine_file in machine_files:
        try:
            result = run_single_experiment(machine_file, args.seed, logger)
            results.append(result)
            if result is not None:
                successful_experiments += 1
            else:
                logger.warning(f"Failed to get results for {machine_file}")
        except Exception as e:
            logger.error(f"Unexpected error with {machine_file}: {e}")
            results.append(None)
    
    logger.info(f"Completed {successful_experiments}/{len(machine_files)} experiments successfully")
    
    if successful_experiments == 0:
        logger.error("No experiments completed successfully")
        cleanup_temp_directories(machine_files, args.seed, logger)
        return 1
    ...
    # Print summary
    logger.info("\n" + "="*60)
    logger.info("SMD MULTI-EXPERIMENT SUMMARY")
    logger.info("="*60)
    logger.info(f"Random Seed: {args.seed}")
    logger.info(f"Experiments completed: {aggregated_result['num_experiments']}/28")
    ...
    logger.info("SMD multi-experiment completed successfully!")
    return 0
