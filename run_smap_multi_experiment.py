#!/usr/bin/env python3
"""
Multi-experiment runner for SMAP dataset (25-feature files only)
Runs experiments on all 54 SMAP files with 25 features and averages the results
"""
...
def setup_logging(seed):
    ...
    log_file = f"results/SMAP/seed_{seed}/logs/smap_multi_experiment_{timestamp}.log"
    
    # Create results directory if it doesn't exist
    os.makedirs(f"results/SMAP/seed_{seed}/logs", exist_ok=True)
...
def get_smap_files():
    """Get SMAP files with 25 features only"""
    from data.smap import SMAPDataset
    
    try:
        smap_files = SMAPDataset.get_smap_files('data/datasets/MSL')
        return smap_files
    except Exception as e:
        print(f"Error getting SMAP files: {e}")
        return []

def run_single_experiment(smap_file, seed, logger):
    """Run experiment on a single SMAP file with 25 features"""
    logger.info(f"Starting experiment for {smap_file} (25 features) with seed {seed}")
    
    # Use single SMAP config file
    config_file = 'configs/SMAP.yaml'
    ...
    # Update config for this specific SMAP file and seed
    config['data']['smap']['filename'] = smap_file
    config['data']['smap']['feature_dim'] = 25
    config['experiment']['name'] = f"smap_{smap_file}_25f_seed_{seed}_experiment"
    config['random_seed'] = seed
    
    # Update result paths to include seed
    config['logging']['log_dir'] = f"./results/SMAP/seed_{seed}/logs"
    config['logging']['model_save_dir'] = f"./results/SMAP/seed_{seed}/models"
    config['logging']['prediction_save_dir'] = f"./results/SMAP/seed_{seed}/predictions"
    
    # Save temporary config
    temp_config_path = f'configs/SMAP_{smap_file}_25f_seed_{seed}.yaml'
    ...
    try:
        ...
        if result.returncode != 0:
            logger.error(f"Experiment failed for {smap_file}")
            logger.error(f"Error output: {result.stderr}")
            return None
        
        logger.info(f"Experiment completed successfully for {smap_file}")
        
        # Immediately copy results to prevent overwriting by next experiment
        predictions_dir = f"results/SMAP/seed_{seed}/SMAP/predictions"
        temp_predictions_dir = f"results/SMAP/seed_{seed}/temp_predictions_{smap_file}_25f"
        ...
        if os.path.exists(predictions_dir):
            ...
            logger.info(f"Copied results for {smap_file} to {temp_predictions_dir}")
        else:
            logger.error(f"Predictions directory not found: {predictions_dir}")
            logger.info(f"Looking for alternative prediction locations...")
            # Try alternative path without nested SMAP
            alt_predictions_dir = f"results/SMAP/seed_{seed}/predictions"
        ...
        return parse_experiment_results_from_dir(smap_file, temp_predictions_dir, logger)
        
    except subprocess.TimeoutExpired:
        logger.error(f"Experiment timed out for {smap_file}")
        return None
    except Exception as e:
        logger.error(f"Error running experiment for {smap_file}: {e}")
        return None
    finally:
...
def parse_experiment_results_from_dir(smap_file, predictions_dir, logger):
    ...
    try:
        ...
        if not pred_files:
            logger.error(f"No prediction results found for {smap_file} in {predictions_dir}")
            return None
        ...
        logger.info(f"Individual results for {smap_file} (25f): AUC={individual_metrics['auc']:.4f}, "
        ...
        logger.error(f"Error parsing results for {smap_file}: {e}")
        return None
...
def aggregate_results(results_list, logger):
    ...
    return {
        ...
        'num_25f_files': len(valid_results),  # All files are 25-feature now
...
def save_aggregated_results(aggregated_result, seed, logger):
    ...
    # Create results directory if it doesn't exist
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    results_dir = f"results/SMAP/seed_{seed}"
    os.makedirs(results_dir, exist_ok=True)
    
    # Save JSON results
    json_file = f"{results_dir}/smap_multi_experiment_results_seed_{seed}_{timestamp}.json"
    ...
    confusion_matrix_data.extend([
        ...
        {'Metric': '25-feature Files', 'Value': aggregated_result['num_25f_files']},
    ])
    
    cm_df = pd.DataFrame(confusion_matrix_data)
    cm_file = f"{results_dir}/smap_confusion_matrix_seed_{seed}_{timestamp}.csv"
    ...
    final_summary_file = f"{results_dir}/smap_final_summary_seed_{seed}_{timestamp}.csv"
    ...
    individual_csv = f"{results_dir}/smap_individual_results_seed_{seed}_{timestamp}.csv"
...
def cleanup_temp_directories(smap_files, seed, logger):
    """Clean up temporary prediction directories"""
    logger.info("Cleaning up temporary directories...")
    
    import shutil
    for smap_file in smap_files:
        temp_dir = f"results/SMAP/seed_{seed}/temp_predictions_{smap_file}_25f"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run multi-experiment for SMAP dataset.")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for experiments.")
    args = parser.parse_args()

    logger = setup_logging(args.seed)
    logger.info(f"Starting SMAP multi-experiment runner with seed {args.seed}")
    
    # Get SMAP files with 25 features
    smap_files = get_smap_files()
    
    if not smap_files:
        logger.error("No SMAP files with 25 features found")
        return 1
    
    logger.info(f"Will run experiments on {len(smap_files)} SMAP files:")
    for i, smap_file in enumerate(smap_files):
        logger.info(f"  File {i+1}: {smap_file}")
    
    logger.info(f"Using random seed: {args.seed}")
    logger.info(f"Results will be saved to: results/SMAP/seed_{args.seed}/")
    ...
    for smap_file in smap_files:
        try:
            result = run_single_experiment(smap_file, args.seed, logger)
            results.append(result)
            if result is not None:
                successful_experiments += 1
            else:
                logger.warning(f"Failed to get results for {smap_file}")
        except Exception as e:
            logger.error(f"Unexpected error with {smap_file}: {e}")
            results.append(None)
    
    logger.info(f"Completed {successful_experiments}/{len(smap_files)} experiments successfully")
    
    if successful_experiments == 0:
        logger.error("No experiments completed successfully")
        cleanup_temp_directories(smap_files, args.seed, logger)
        return 1
    ...
    # Print summary
    logger.info("\n" + "="*60)
    logger.info("SMAP MULTI-EXPERIMENT SUMMARY")
    logger.info("="*60)
    logger.info(f"Random Seed: {args.seed}")
    logger.info(f"Experiments completed: {aggregated_result['num_experiments']}/54")
    logger.info(f"  25-feature files: {aggregated_result['num_25f_files']}")
    ...
    logger.info("SMAP multi-experiment completed successfully!")
    return 0
