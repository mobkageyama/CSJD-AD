#!/usr/bin/env python3
"""
Multi-experiment runner for ECG dataset
Runs experiments on all 9 ECG files and averages the results
"""
...
def setup_logging(seed):
    ...
    log_file = f"results/ECG/seed_{seed}/logs/ecg_multi_experiment_{timestamp}.log"
    
    # Create results directory if it doesn't exist
    os.makedirs(f"results/ECG/seed_{seed}/logs", exist_ok=True)
...
def run_single_experiment(ecg_file, seed, logger):
    """Run experiment on a single ECG file"""
    logger.info(f"Starting experiment for {ecg_file} with seed {seed}")
    
    # Create a temporary config file for this experiment
    with open('configs/ECG.yaml', 'r') as f:
        config = yaml.safe_load(f)
    
    # Update config for this specific ECG file and seed
    config['data']['ecg']['filename'] = ecg_file
    config['experiment']['name'] = f"ecg_{ecg_file}_seed_{seed}_experiment"
    config['random_seed'] = seed
    
    # Update result paths to include seed
    config['logging']['log_dir'] = f"./results/ECG/seed_{seed}/logs"
    config['logging']['model_save_dir'] = f"./results/ECG/seed_{seed}/models"
    config['logging']['prediction_save_dir'] = f"./results/ECG/seed_{seed}/predictions"
    
    # Save temporary config
    temp_config_path = f'configs/ECG_{ecg_file}_seed_{seed}.yaml'
    ...
    try:
        ...
        if result.returncode != 0:
            logger.error(f"Experiment failed for {ecg_file}")
            logger.error(f"Error output: {result.stderr}")
            return None
        
        logger.info(f"Experiment completed successfully for {ecg_file}")
        
        # Immediately copy results to prevent overwriting by next experiment
        # The actual predictions are saved in a nested ECG directory due to how run.py handles paths
        predictions_dir = f"results/ECG/seed_{seed}/ECG/predictions"
        temp_predictions_dir = f"results/ECG/seed_{seed}/temp_predictions_{ecg_file}"
        ...
        if os.path.exists(predictions_dir):
            ...
            logger.info(f"Copied results for {ecg_file} to {temp_predictions_dir}")
        else:
            logger.error(f"Predictions directory not found: {predictions_dir}")
            logger.info(f"Looking for alternative prediction locations...")
            # Try alternative path without nested ECG
            alt_predictions_dir = f"results/ECG/seed_{seed}/predictions"
        ...
        return parse_experiment_results_from_dir(ecg_file, temp_predictions_dir, logger)
        
    except subprocess.TimeoutExpired:
        logger.error(f"Experiment timed out for {ecg_file}")
        return None
    except Exception as e:
        logger.error(f"Error running experiment for {ecg_file}: {e}")
        return None
    finally:
...
def parse_experiment_results_from_dir(ecg_file, predictions_dir, logger):
    ...
    try:
        ...
        if not pred_files:
            logger.error(f"No prediction results found for {ecg_file} in {predictions_dir}")
            return None
        ...
        logger.info(f"Individual results for {ecg_file}: AUC={individual_metrics['auc']:.4f}, "
        ...
        logger.error(f"Error parsing results for {ecg_file}: {e}")
        return None
...
def save_aggregated_results(aggregated_result, seed, logger):
    ...
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    results_dir = f"results/ECG/seed_{seed}"
    
    # Create results directory if it doesn't exist
    os.makedirs(results_dir, exist_ok=True)
    
    # Save detailed JSON results
    json_file = f"{results_dir}/ecg_multi_experiment_results_seed_{seed}_{timestamp}.json"
    ...
    cm_file = f"{results_dir}/ecg_confusion_matrix_seed_{seed}_{timestamp}.csv"
    ...
    final_summary_file = f"{results_dir}/ecg_final_summary_seed_{seed}_{timestamp}.csv"
    ...
    individual_csv = f"{results_dir}/ecg_individual_results_seed_{seed}_{timestamp}.csv"
...
def cleanup_temp_directories(ecg_files, seed, logger):
    """Clean up temporary prediction directories"""
    logger.info("Cleaning up temporary directories...")
    
    import shutil
    for ecg_file in ecg_files:
        temp_dir = f"results/ECG/seed_{seed}/temp_predictions_{ecg_file}"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run multi-experiment for ECG dataset.")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for experiments.")
    args = parser.parse_args()

    logger = setup_logging(args.seed)
    logger.info(f"Starting ECG multi-experiment runner with seed {args.seed}")
    
    # List of all ECG files (based on available files in data/datasets/ECG/labeled/whole/)
    ecg_files = [
        "chfdb_chf01_275",
        "chfdb_chf13_45590", 
        "chfdbchf15",
    ...
    logger.info(f"Will run experiments on {len(ecg_files)} ECG files: {ecg_files}")
    logger.info(f"Using random seed: {args.seed}")
    logger.info(f"Results will be saved to: results/ECG/seed_{args.seed}/")
    ...
    for ecg_file in ecg_files:
        try:
            result = run_single_experiment(ecg_file, args.seed, logger)
            results.append(result)
            if result is not None:
                successful_experiments += 1
            else:
                logger.warning(f"Failed to get results for {ecg_file}")
        except Exception as e:
            logger.error(f"Unexpected error with {ecg_file}: {e}")
            results.append(None)
    
    logger.info(f"Completed {successful_experiments}/{len(ecg_files)} experiments successfully")
    
    if successful_experiments == 0:
        logger.error("No experiments completed successfully")
        cleanup_temp_directories(ecg_files, args.seed, logger)
        return 1
    ...
    # Print summary
    logger.info("\n" + "="*60)
    logger.info("ECG MULTI-EXPERIMENT SUMMARY")
    logger.info("="*60)
    logger.info(f"Random Seed: {args.seed}")
    logger.info(f"Experiments completed: {aggregated_result['num_experiments']}/9")
